...
注意，不能写成 `export StringValidator` 。
...
虽然下面的做法是不推荐的，但是有些场景下，一些模块需要单独设置相关全局变量以供其他模块使用。
...
注意，只有在目标代码模块为 CommonJS 和 AMD 才能使用此做法。
...
如果一个模块导出对象在被引用时，**仅仅**被用在类型注解中，而**不在任何表达式**中，那么在生成的代码中就不会产生对此模块的 `require` 调用。
...
为了描述不是使用 TS 写的第三方库，我们可以使用**声明**（Declaration）来描述库所暴露的 API 。
...
一些模块加载器如 SystemJS 和 AMD 允许引入非 JS 内容。
它们的做法通常是使用前缀或者后缀来表明特殊内容的加载方式。
**通配符模块声明**可用于这种场景。
